...
const WorkGuidelines = ({
  ...
  handlePrev,
  handleNext,
  currentAssignGuidelines,
  selectedSectionIds,
  toggleAssign,
  handleSave,
  isSaving,
  selectedSections,
  setAssignMap, // 必要に応じて親から渡す
}) => {
  ...
  useEffect(() => {
    const loadPast = async () => {
      ...
      if ((selectedPeriod || periodKeys[0]) && past[selectedPeriod || periodKeys[0]]) {
        const ids = past[selectedPeriod || periodKeys[0]].guidelines[selectedGrade] || [];
        setPastSections((grouped["働き方の指針"] || []).filter(s => ids.includes(s.id)));
      }
    };
    loadPast();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedGrade, grouped, selectedPeriod]);
  ...
  useEffect(() => {
    const fetchSections = async () => {
      ...
      if (selectedPeriod && past[selectedPeriod]) {
        const ids = past[selectedPeriod].guidelines[selectedGrade] || [];
        setPastSections((grouped["働き方の指針"] || []).filter(s => ids.includes(s.id)));
      }
    };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedPeriod, selectedGrade, grouped, pastAssignments]);


  // 「コピー」ボタンで選択中の期の割り当てをコピー
  const handleCopySelectedPeriod = () => {
    if (!selectedPeriod || !pastAssignments[selectedPeriod]) return;
    const copyIds = pastAssignments[selectedPeriod].guidelines[selectedGrade] || [];
    setAssignMap(prev => ({
      ...prev,
      [`guidelines_${selectedGrade}`]: Object.fromEntries(
        (grouped["働き方の指針"] || []).map(section => [
          section.id,
    ...
    <div className="flex h-[80vh] p-4 gap-8">
      ...
      <div className="w-1/2 overflow-y-auto border-r pr-6 pl-3">
        <div className="flex items-center gap-4 mb-4">
          <Button size="icon" variant="ghost" onClick={() => handlePrev(true)} disabled={allGrades.length === 0}>
            <FiChevronLeft />
          </Button>
          <div className="text-lg font-bold">
            等級: {selectedGrade}
          </div>
          <Button size="icon" variant="ghost" onClick={() => handleNext(true)} disabled={allGrades.length === 0}>
            <FiChevronRight />
          </Button>
        </div>
          ...
          {(grouped["働き方の指針"] || []).map((section) => (
            <li
              ...
              className={`p-4 rounded-lg shadow border flex flex-col bg-white ${
                currentAssignGuidelines[section.id] ? "ring-2 ring-blue-400" : ""
              }`}
            >
              <div className="flex items-center justify-between">
                ...
                <Button
                  size="sm"
                  variant={currentAssignGuidelines[section.id] ? "default" : "outline"}
                  onClick={() => toggleAssign(section.id, true)}
                >
                  {currentAssignGuidelines[section.id] ? "割り当て解除" : "割り当て"}
                </Button>
              </div>
      ...
      <div className="w-1/2 pl-6 flex flex-col">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="ml-2 text-sm text-gray-600">等級: {selectedGrade}</span>
        </div>
